...
int main(){
	...
	//cv::threshold(cimgl,gimgl,0,255,CV_THRESH_BINARY|CV_THRESH_OTSU);
	//cv::threshold(cimgl,gimgl,thr,0,THRESH_TOZERO);
	...
	std::cout << "cimg.rows : " << cimgl.rows << std::endl;
	std::cout << "cimg.cols : " << cimgl.cols<< std::endl;
	std::cout << "gimg.rows : " << gimgl.rows << std::endl;
	std::cout << "gimg.cols : " << gimgl.cols<< std::endl;
	...
	for(int i=0;i<cimgl.rows;i++){
		for(int j=0;j<cimgl.cols;j++){
			...
			int pixel = gimgl.at<unsigned char>(i,j);
			//unsigned char pixel = gimgl.at<uchar>(i,j);
	...
	std::cout <<"most brightness" << max << std::endl;
	std::cout << max_num_i << std::endl;
	std::cout << max_num_j << std::endl;

	for(int i=0;i<gimgl.rows;i++){
		for(int j=0;j<gimgl.cols;j++){
			if( i < max_num_i +squ &&  i > max_num_i -squ  && (j < squ+ max_num_j && j >max_num_j-squ ))

				gimgl.at<unsigned char>(i,j) = 255;	

			else

				gimgl.at<unsigned char>(i,j) = 0;	
